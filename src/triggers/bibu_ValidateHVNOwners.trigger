trigger bibu_ValidateHVNOwners on HVN__c (before insert, before update) {
...
//    List<AccountTeamMember> atm = new List<AccountTeamMember>(); 
    ...
    for(HVN__c hvn : trigger.new){
        ...
        if(!contacts.isEmpty()){
            ...
            if(hvn.Name!=cnt.Name)
                hvn.Name.addError('HVN contact name should be same as Contact name');
    
            if(hvn.Gift_Currency__c!=null)
                hvn.CurrencyIsoCode = hvn.Gift_Currency__c.substring(0, 3);
...
//              if(atm==null || atm.size()==0){
                nAccTeamCount = [Select count() From AccountTeamMember where TeamMemberRole='HVN Owner' and IsDeleted=false and AccountId=:cnt.AccountId and UserId=:hvn.Secondary_HVN_Contact_Owner_1__c];
                if(nAccTeamCount==0){
...
//              if(atm==null || atm.size()==0){
                nAccTeamCount = [Select count() From AccountTeamMember where TeamMemberRole='HVN Owner' and IsDeleted=false and AccountId=:cnt.AccountId and UserId=:hvn.Secondary_HVN_Contact_Owner_2__c];
                if(nAccTeamCount==0){
...
//              if(atm==null || atm.size()==0){
                nAccTeamCount = [Select count() From AccountTeamMember where TeamMemberRole='HVN Owner' and IsDeleted=false and AccountId=:cnt.AccountId and UserId=:hvn.Secondary_HVN_Contact_Owner_3__c];
                if(nAccTeamCount==0){
